...
export const actions = {
  ...
  updateResource({dispatch, commit},payload){
    return new Promise(((resolve, reject) => {
      ...
      if(payload.data.id){
        this.$axios.put(`/${payload.resource}/${payload.data.id}`, payload.data)
        ...
        this.$axios.post(`/${payload.resource}`, payload.data)
          .catch(error => reject(error))
          .then((response) => {
            dispatch('misc/replyReceived', {success: true},{root: true})
            dispatch('syncData')
  ...
  deleteResource({dispatch},payload){
    dispatch('misc/requestSend', null,{root: true})
    return new Promise((resolve,reject) => {
      this.$axios.delete(`/${payload.resource}/${payload.data.id}`)
        .catch(error => reject(error))
        .then((response) => {
          dispatch('misc/replyReceived', {success: true},{root: true})
          dispatch('misc/makeToast',{type: "delete",variant: "success"},{root: true})
          dispatch('syncData')
          resolve(response.data)
        })
    })
