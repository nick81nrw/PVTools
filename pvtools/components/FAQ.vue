...
function getRealms() {
  return faqs.value.reduce(
    (prev, curr) => (prev.includes(curr.realm) ? prev : [...prev, curr.realm]),
    []
  )
}

function getFaqs(realm) {
  return faqs.value.filter((faq) => faq.realm == realm)
}

function toggleCollapses(id) {
  $root.$emit('bv::toggle::collapse', id)
}
</script>
